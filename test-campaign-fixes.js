...
async function testLeadLimit() {
  ...
  try {
    ...
    log(`✓ Campaign created: ${campaign.id}`, 'green');
    ...
    log(`✓ Steps added: ${stepResponse.data.data.length} steps`, 'green');

    // Start campaign
    log('Starting campaign...', 'yellow');
    await api.post(`/api/campaigns/${campaign.id}/start`);
    log('✓ Campaign started', 'green');
    ...
    if (leadsCount === 3) {
      log('✓ TEST PASSED: Exactly 3 leads generated', 'bright');
    } else {
      log(`✗ TEST FAILED: Expected 3 leads, got ${leadsCount}`, 'red');
    }
...
async function testConnectionWithMessage() {
  ...
  try {
    ...
    log(`✓ Added connection message: "${message}"`, 'green');
    ...
    log('✓ Check backend logs for:', 'cyan');
...
async function testConnectionWithoutMessage() {
  ...
  try {
    ...
    await api.post(`/api/campaigns/${campaign.id}/steps`, stepsData);
    log('✓ Added connection step with message: null', 'green');
...
async function testApolloEnrichment() {
  ...
  try {
    ...
    await api.post(`/api/campaigns/${campaign.id}/steps`, stepsData);
    log('✓ Added lead generation step', 'green');
...
async function runAllTests() {
  ...
  log('\n💡 MANUAL VERIFICATION NEEDED:', 'cyan');
