 ...
 * POST /api/social-integration/calendar/microsoft/start
 */
router.post('/microsoft/start', authenticateToken, async (req, res) => {
  try {
    const { user_id, frontend_id } = req.body;
    const userId = user_id || req.user?.id;
    ...
    // Call VOAG service to initiate OAuth
    const voagResponse = await axios.post(
      `${VOAG_BASE_URL}/auth/microsoft/start?user_id=${userId}&frontend_id=${frontend_id || 'settings'}&redirect_uri=${encodeURIComponent(redirectUri)}`,
      {},
      {
        headers: {
          'Content-Type': 'application/json',
    ...
    logger.info('[Calendar] VOAG Microsoft OAuth response', { 
      status: voagResponse.status,
      hasUrl: !!voagResponse.data?.url 
    });
    ...
    logger.error('[Calendar] Error starting Microsoft OAuth', {
      error: error.message,
      response: error.response?.data,
      status: error.response?.status
    });
    ...
    res.status(error.response?.status || 500).json({
      error: error.response?.data?.error || 'Failed to start Microsoft OAuth flow',
      message: error.message
    });
  }
});
 ...
 * POST /api/social-integration/calendar/microsoft/status
 */
router.post('/microsoft/status', authenticateToken, async (req, res) => {
  try {
    const { user_id } = req.body;
    const userId = user_id || req.user?.id;
    ...
    // Call VOAG service to check status
    const voagResponse = await axios.get(
      `${VOAG_BASE_URL}/auth/microsoft/status?user_id=${userId}`,
      {
        headers: {
          'Authorization': req.headers.authorization || '',
          'X-Frontend-ID': 'settings',
    ...
    logger.error('[Calendar] Error checking Microsoft status', {
      error: error.message,
      response: error.response?.data
    });
 ...
 * POST /api/social-integration/calendar/microsoft/disconnect
 */
router.post('/microsoft/disconnect', authenticateToken, async (req, res) => {
  try {
    const { user_id } = req.body;
    const userId = user_id || req.user?.id;
    ...
    logger.info('[Calendar] Disconnecting Microsoft Calendar', { userId });

    // Call VOAG service to disconnect
    const voagResponse = await axios.post(
      `${VOAG_BASE_URL}/auth/microsoft/disconnect?user_id=${userId}`,
      {},
      {
        headers: {
          'Content-Type': 'application/json',
    ...
    logger.error('[Calendar] Error disconnecting Microsoft', {
      error: error.message,
      response: error.response?.data
    });

    res.status(error.response?.status || 500).json({
      error: error.response?.data?.error || 'Failed to disconnect Microsoft Calendar',
      message: error.message
    });
  }
});
