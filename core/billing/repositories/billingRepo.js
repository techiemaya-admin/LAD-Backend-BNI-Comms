...
async function getOrCreateWallet(tenantId, forUpdate = false) {
  ...
  if (!forUpdate) {
    ...
    sql = `
      INSERT INTO ${schema}.billing_wallets (tenant_id, user_id, current_balance, currency, status)
      SELECT $1, NULL, 0, 'USD', 'active'
      WHERE NOT EXISTS (
        SELECT 1 FROM ${schema}.billing_wallets WHERE tenant_id = $1 AND user_id IS NULL
      )
      RETURNING *
    `;
...
async function getLedgerByIdempotencyKey(tenantId, idempotencyKey) {
  const schema = getSchema();
  const sql = `
    SELECT * FROM ${schema}.billing_ledger_transactions
...
async function listLedgerTransactions({ tenantId, walletId, fromDate, toDate, limit = 100, offset = 0 }) {
  ...
  let sql = `
    ...
    LEFT JOIN ${schema}.billing_wallets w ON lt.wallet_id = w.id
    LEFT JOIN ${schema}.users u ON lt.created_by = u.id
...
async function createUsageEvent({
  ...
  const schema = getSchema();
  const sql = `
    INSERT INTO ${schema}.billing_usage_events (
...
async function getUsageByIdempotencyKey(tenantId, idempotencyKey) {
  const schema = getSchema();
  const sql = `
    SELECT * FROM ${schema}.billing_usage_events
    WHERE tenant_id = $1 AND idempotency_key = $2
  `;
...
async function updateUsageEventStatus({
  ...
  const schema = getSchema();
  const sql = `
    UPDATE ${schema}.billing_usage_events
...
async function listUsageEvents({
  ...
  let sql = `
    ...
    FROM ${schema}.billing_usage_events ue
    LEFT JOIN ${schema}.users u ON ue.user_id = u.id
...
async function getUsageAggregation({ tenantId, featureKey, fromDate, toDate }) {
  ...
  let sql = `
    SELECT 
      ...
      currency
    FROM ${schema}.billing_usage_events
    WHERE tenant_id = $1
  `;
...
async function getFeatureEntitlement(tenantId, featureKey) {
  const schema = getSchema();
  const sql = `
    SELECT * FROM ${schema}.billing_feature_entitlements
