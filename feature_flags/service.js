...
class FeatureFlagService {
   ...
   * Works with dynamic schema using tenant_id column
   */
  ...
  async enableFeature(clientId, featureKey) {
    try {
      const schema = getSchema();
      const queryText = `
        INSERT INTO ${schema}.client_features (client_id, feature_id, enabled)
        SELECT $1, f.id, true
  ...
  async disableFeature(clientId, featureKey) {
    try {
      const schema = getSchema();
      const queryText = `
        UPDATE ${schema}.client_features
        SET enabled = false, updated_at = NOW()
        WHERE client_id = $1 
        AND feature_id = (SELECT id FROM ${schema}.features WHERE key = $2)
      `;
